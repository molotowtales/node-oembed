...
DiscoveryHandler.prototype = {
    ...
    if (el.name === 'link' &&
        (attrs.rel === 'alternate' || attrs.rel === 'alternative') &&
        type && href) {
...
exports.applyParameters = function(url, parameters) {
    url = url.replace(/&#x3D;/gi, '=');
    if (!parameters)
    /* Nothing to do, skip */
    return url;
...
exports.fetch = function(url, parameters, cb) {
    ...
    if (alternates && alternates[MIME_OEMBED_JSON]) {
        oembedUrl = exports.applyParameters(alternates[MIME_OEMBED_JSON], parameters);
        exports.fetchJSON(oembedUrl, cb);
    } else if (alternates && alternates[MIME_OEMBED_XML]) {
        oembedUrl = exports.applyParameters(alternates[MIME_OEMBED_XML], parameters);
        ...
        oembedUrl = exports.applyParameters(exports.EMBEDLY_URL, parameters);
        exports.fetchJSON(oembedUrl, cb);
    } else
